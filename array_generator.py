...
geo_x = np.zeros((int(total_geo_num),), dtype = np.float32)
geo_y = np.zeros((int(total_geo_num),), dtype = np.float32)
geo_z = np.zeros((int(total_geo_num),), dtype = np.float32)
for i in range(ARM_NUM):
    for j in range(int(NUM_GEO_PER_ARM)):
